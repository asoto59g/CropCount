...
def simplify_contour(contour: np.ndarray, minimum_percent: float, image_shape: tuple[int, int]) -> np.ndarray:
    ...
    epsilon = diagonal * minimum_percent / 100.0
...
def radial_signature(mask: np.ndarray, center: tuple[float, float], samples: int = 720) -> tuple[np.ndarray, np.ndarray]:
    ...
    for index, angle in enumerate(angles):
        ...
        if occupied_indices.size:
            values[index] = radii[occupied_indices[-1]]
...
simplified = max(simplified_contours, key=cv2.contourArea)
center = contour_centroid(simplified)
angles, radii = radial_signature(mask, center)
overlay = overlay_contours(rgb, contours, simplified_contours, center)
vertex_count = sum(len(item) for item in simplified_contours)
...
metric_4.metric("Vértices vectoriales", f"{vertex_count:,}")
...
csv_lines = ["angle_deg,radius_px,radius_normalized"]
normalizer = max(float(np.hypot(rgb.shape[1], rgb.shape[0])), 1.0)
csv_lines.extend(f"{angle:.3f},{radius:.3f},{radius / normalizer:.6f}" for angle, radius in zip(angles, radii))
...
with export_1:
    st.download_button("Descargar contorno SVG", svg_data, "contorno_palmera.svg", "image/svg+xml", use_container_width=True)
with export_2:
    st.download_button("Descargar firma CSV", csv_data, "firma_radial_palmera.csv", "text/csv", use_container_width=True)
